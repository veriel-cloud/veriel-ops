...
export function Timeline({ steps }: TimelineProps) {
  const currentStep = steps.findIndex((s) => s.status === "loading" || s.status === "pending");
    ...
    <div>
            ...
            <div key={i} className="flex gap-3">
              <div className="flex flex-col items-center">
                <div className={cn(
                  ...
                  step.status === "success" && "bg-[var(--color-success-light)] border border-[var(--color-success)]/20",
                  ...
                  {step.status === "success" && (
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 text-[var(--color-success)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M5 12l5 5l10 -10" />
                    </svg>
                  )}
                  {step.status === "error" && (
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3 text-[var(--color-error)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className={cn("pb-5 pt-0.5", isLast && "pb-0")}>
                <div className="flex items-center gap-2">
                  ...
                  {step.duration !== undefined && step.status === "success" && (
                    <span className="text-[11px] text-[var(--color-text-quaternary)] tabular-nums">{(step.duration / 1000).toFixed(1)}s</span>
                  )}
                </div>
                {step.detail && (
                  <p className="text-[11px] text-[var(--color-text-quaternary)] mt-0.5">{step.detail}</p>
                )}
              </div>
            </div>
          );
      ...
      {totalDone > 0 && totalDone < steps.length && (
        <div className="mt-3 pt-3 border-t border-[var(--color-border)]">
          <p className="text-[11px] text-[var(--color-text-quaternary)]">Paso {totalDone + 1} de {steps.length}</p>
        </div>
      )}
    </div>
  );
