...
export function Dashboard() {
  ...
  const totalBuilds = projects.reduce(
    (sum: number, p: any) => sum + (p.builds?.des ?? 0) + (p.builds?.pre ?? 0) + (p.builds?.pro ?? 0),
    0,
  );
    ...
    <>
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link
            to="/projects"
            className="text-[12px] text-[var(--color-text-quaternary)] hover:text-[var(--color-text-secondary)] transition-colors"
          >
            View all →
          </Link>
        </div>
      ...
      <section>
        ...
        <Card padding={false}>{ld ? <SkeletonTable rows={5} /> : <DeployTable deploys={deploys} limit={6} />}</Card>
      </section>
    </>
  );
}
